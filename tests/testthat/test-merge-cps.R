...
test_that("merge_cps works with good data", {
  ...
  load("./test-hdr4-data/hdr4-test-spectra.rda")

  expect_no_error(good.cps_spct <- raw2corr_cps(good_hdr.raw_spct, ref.pixs.range = 2:4))
...
test_that("merge_cps works with bad data", {
  ...
  expect_message(bad.cps_spct <- raw2corr_cps(bad_hdr.raw_spct, ref.pixs.range = 2:4))
