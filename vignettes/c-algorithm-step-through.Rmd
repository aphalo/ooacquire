...
We will step through the different steps of the algorithm used, plotting the spectra at each step along the way. We use raw counts data, included in the package, to make building this vignette easier. At the same time we show how raw data acquired outside R can be also processed with the package.
...
white_LED.raw_mspct
```
Plotting the three bracketed spectra shows the raw data. The detector saturates at approximately 64000 counts.
...
Next we need to apply a linearisation function, as detector response is not linear---saturation is gradual.
...
We may next remove an estimate of dark signal based on reference pixels not expected to be excited. This step may appear redundant, but _might_ help in cases when instrument temperature is not stable.
...
We subtract the dark reading from both the light and filter measurements, and copy attributes, including instrument settings and calibration data.
...
Step by step application of slit function tail correction.
