...
on the argumentes passed to parameters of the function.
...
: Timing of the acquisition of spectra. Buffered acquisition is as fast as the spectromter allows, with the frequency controlled by the total integration time plus an overhead < 1 ms. In the case of Buffered acquisition there is minimal dead time between acquisitions. Fast acquisition has more dead time as settings in the spectrometer need to we modified two or more times per measuring event. While Buffered and Fast series are acquired as fast as possible, in the case of a timed series, spectra are retrieved at specific times, with dead time in-between.  
