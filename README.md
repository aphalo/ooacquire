...
off-line to read raw-counts data from files saved by Ocean Optics’s
...
data from files obtained with other software from Ocean Optics are
...
with software from Ocean Optics can be processed without the need to
install packages **‘rOmniDriver’** and **‘rJava’** or the **OmniDriver**
drivers from Ocean Optics. Ocean Optics’s **SPAM** library is not used
...
in the header of data files created with software from Ocean Optics.
...
no need to install Ocean Optics’ OmniDriver or package ‘rOmniDriver’.
...
Calibrations are linked to individual spectrometers through the
instrument serial number, and as they need to be updated periodically,
multiple calibrations are usually in use for a given instrument and
selected depending on the data acquisition date.

The package includes calibration data for the spectrometers used in
testing the package or used by myself and collaborators. Before
‘ooacquire’ can be used with other spectrometers for measuring
irradiance, calibration data and their “instrument descriptor” has to be
available.

Data acquisition functions from ‘ooacquire’ fall-back onto retrieving
irradiance calibration data from the spectrometer EEPROM when no other
calibration or correction methods are available. In such case, only a
simple correction algorithm is available, not different from that used
by OceanOptics own spectrometer support software.
...
the same for linearisation as the polynomial coefficients are also
retrievable from the spectrometer.

*Depending on how detailed has been the characterization of the
spectrometer different corrections are available. The source package
includes a folder ‘data-raw’ with examples of how correction methods’
parameters and functions can be made availble. The process cannot be
easily automated as bad pixels and suitable reference wavelengths need
to be chosen based both on the instrument used and light source to be
measured. Ylianttila et al. (2005) describe correction methods on which
those in ‘ooacuire’ are based, as well as their performance.*

## Documentation

HTML documentation for this package is available at
(<https://docs.r4photobiology.info/ooacquire/>). Documentation includes
4 package vignettes and 1 on-line-only articles, in addition to help
pages.

The examples in the vignettes and help pages use spectral data from
...
The API documentation for the *OmniDriver SDP* is no longer available
from Ocean Optics.
...
`pckgdown` contains bitmaps and other files used in the HTML version of
...
Ylianttila, Lasse, Visuri, Reijo, Huurto, Laura, Jokela, Kari (2005)
Evaluation of a single-monochromator diode array spectroradiometer for
sunbed UV-radiation measurements. *Photochemistry and Photobiology* ,
81(2), 333-341.
...
(<https://oceanoptics.com/>). Although still to some extent maintained,
*OmniDriver* development has been discontinued by OceanOptics.
