...
#'
#' @param FEL_raw_counts source_mscpt The raw counts and integration time data
#'   for the FEL light source.
#' @param D2_raw_counts source_mscpt The raw counts and integration time data
#'   for the D2 light source.
#' @param pix_wavelengths numeric vector The "true" wavelengths in nanometres at
#'   each pixel in the detector array.
#' @param wl_range numeric vector of length two Range of wavelengths for which
#'   to compute the calibration.
#' @param FEL_k numeric vector a numeric vector with n constants for the
#'   polynomial for the FEL lamp.
#' @param D2_k numeric vector a numeric vector with n constants for the
...
compute_irrad_calibration <-
  function(FEL_raw_counts,
           D2_raw_counts,
           pix_wavelengths = NULL,
           wl_range = c(250, 900),
           FEL_k,
           D2_k,
           method,
           verbose = FALSE)
{
    FEL_spct <- s_irrad_corrected(FEL_raw_counts,
                                  method = method,
                                  return.cps = TRUE,
                                  verbose = verbose)

    D2_spct <- s_irrad_corrected(D2_raw_counts,
                                 method = method,
                                 return.cps = TRUE,
                                 verbose = verbose)
    ...
    if (!is.null(pix.wavelengths)) {
      FEL_spct[["w.length"]] <- pix_wavelengths
      D2_spct[["w.length"]] <- pix_wavelengths
    } else {
      warning("Using 'w.length' from measured spectrum for calibration.")
      pix.wavelengths <- FEL_spct[["w.length"]]
    }

    within_range <- pix.wavelengths >= wl_range[1] &
                       pix.wavelengths <= wl_range[2]

    FEL_calib_spct <- photobiology::FEL_spectrum(pix_wavelengths, FEL_k, fill = NA)
    FEL_mult <- ifelse(within_range,
                      FEL_spct[["cps"]] / FEL_calib_spct[["s.e.irrad"]], NA)

    D2_spct <- photobiology::D2_spectrum(pix.wavelengths, D2_k, fill = NA)

    D2_calib_spct <- photobiology::D2_spectrum(pix_wavelengths, D2_k, fill = NA)
    D2_mult <- ifelse(within_range,
                      D2_spct[["cps"]] / D2_calib_spct[["s.e.irrad"]], NA)

    ovelaping <- pix_wavelengths >= 330 & pix_wavelengths <= 360

    D2_corr_factor <- mean(FEL_mult[ovelaping] / D2_mult[ovelaping], na.rm=TRUE)

    D2_mult <- D2_mult * D2_corr_factor

    irrad_mult <- rep(0, length.out = length(pix_wavelengths))
    irrad_mult[within_range] <-
      ifelse(pix_wavelengths[within_range] <= 340,
    ...
    set_descriptor_wl(oo_descriptor = updated_descriptor,
                      wl = pix.wavelengths)
    set_descriptor_irrad_mult(oo_descriptor = updated_descriptor,
                              irrad.mult = irrad_mult,
                              wl.range = wl_range,
                              start.date = when.measured,
                              end.date = when.measured + lubridate::years(2))
    comment(updated_descriptor) <-
      paste("'ooacquire' version ", package_version("ooacquire"),
            " on ", lubridate::now(), "\n\nFEL spectrum:\n", comment(FEL_spct),
            "\n\nD2 spectrum:\n", comment(D2_spct),
            sep="")
