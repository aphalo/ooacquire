...
get_oo_descriptor <- function(w,
  ...
  list(
    ...
    spectrometer.sn =  rOmniDriver::get_serial_number(jwrapper = w,
                                                      sr.index = sr.index),
    bench.grating = ifelse(class(bench) != "jobjRef" || !methods::is(bench, "jobjRef"),
                           NA_character_, bench$getGrating()),
    bench.filter = ifelse(class(bench) != "jobjRef" || methods::is(bench, "jobjRef"),
                          NA_character_, bench$getFilterWavelength()),
    bench.slit = ifelse(class(bench) != "jobjRef" || !methods::is(bench, "jobjRef"),
