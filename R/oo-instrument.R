...
get_oo_descriptor <- function(w,
  ...
  list(
    ...
    spectrometer.sn =  rOmniDriver::get_serial_number(jwrapper = w,
                                                      sr.index = sr.index),
    bench.grating = ifelse(is.na(bench) || !methods::is(bench, "jobjRef"),
                           NA_character_, bench$getGrating()),
    bench.filter = ifelse(is.na(bench) || methods::is(bench, "jobjRef"),
                          NA_character_, bench$getFilterWavelength()),
    bench.slit = ifelse(is.na(bench) || !methods::is(bench, "jobjRef"),
