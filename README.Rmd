...
<!-- badges: start -->
[![R Universe version](https://aphalo.r-universe.dev/badges/ooacquire)](https://aphalo.r-universe.dev/ooacquire)
...
read raw-counts data from files saved by Ocean Optics's software and hardware,
...
Package **'ooacquire'** supports most types of *Ocean Optics* spectrometers
(<https://www.oceanoptics.com/>). The 64 bit versions of the free runtime of the
*OmniDriver SDP* and *Java* need both to be installed before data acquisition is
possible. The driver is only needed for live communication, control and data
acquisition from within R.
...
while maintenance updates are still regularly released. Multiple versions of
OmniDriver are currently available: 2.80, 2.79, 2.75 and 2.56. Recently released
versions do not support very old spectrometer models such as the USB2000 and
HR2000. Version 2.56 is the most recent one that supports them. Package
'ooacquire' works with all versions that I have tested (2.56, 2.75 and several
other).

Package 'ooacquire' can be also used without
installing OmniDriver, but in this case only off-line processing of spectra and
import of spectral data from files obtained with other software from Ocean Optics
are possible. All functions that connect to spectrometer hardware are
disabled in the absence of an installed OmniDriver driver, while all functions
that do not communicate with the spectrometer remain fully functional.

Package 'ooacquire' accesses spectrometers through functions in R package
...
versions of R are only available as **64 bit** binaries. **The versions of the
Java OpenJDK and of OmniDriver to be installed are the 64 bit ones.**
...
to install Ocean Optics' OmniDriver or package 'rOmniDriver'.
...
Calibrations are linked to individual spectrometers through the instrument serial number, and as they need to be updated periodically, multiple calibrations are usually in use for a given instrument and selected depending on the data acquisition date. 
...
'ooacquire' can be used with other spectrometers for measuring irradiance,
calibration data and their "instrument descriptor" has to be available.

Data acquisition functions from 'ooacquire' fall-back onto retrieving irradiance
calibration data from the spectrometer EEPROM when no other calibration or
correction methods are available. In such case, only a simple correction
algorithm is available, not different from that used by OceanOptics own
spectrometer support software.
...
always retrievable from the spectrometer. The case is the same for linearisation
as the polynomial coefficients are also retrievable from the spectrometer.

_Depending on how detailed has been the characterization of the spectrometer
different corrections are available. The source package includes a folder
'data-raw' with examples of how correction methods' parameters and functions can
be made availble. The process cannot be easily automated as bad pixels and
suitable reference wavelengths need to be chosen based both on the instrument
used and light source to be measured. Ylianttila et al. (2005) describe 
correction methods on which those in 'ooacuire' are based, as well as their
performance._

## Documentation

HTML documentation for this package is available at
(<https://docs.r4photobiology.info/ooacquire/>). Documentation includes `r length(list.files(path = "./vignettes", pattern = "*.Rmd"))` package vignettes and `r length(list.files(path = "./vignettes/articles", pattern = "*.Rmd"))` on-line-only articles, in addition to help pages. 

The examples in the vignettes and help pages use spectral data from measurements
done with this package as well as output files created by Ocean Optics's
software. These data files are in folder `extdata`. Scripts containing examples
that can be in most cases used with only small edits are in folder
`example-scripts` of this package.

The API documentation for the *OmniDriver SDP* is no longer available from Ocean Optics.
...
scripts.path <- system.file("extdata", "example-scripts", package = "ooacquire")
...
The folder structure of the Git repository is shown below. Folders `pkgdown`, `inst-not`, `data-raw` and `tests-scripts-interactive` are not included in the built package. Folder `data-raw` also contains raw calibration data and R scripts used to convert them into the objects included in folder `data`. Folder `tests-scripts-interactive` contains R scripts used to test data acquisition with specific spectrometers used during development. Other folders are the usual ones for R packages: `src` contains C++ code sources based on R package 'Rcpp', folder `pckgdown` contains bitmaps and other files used in the HTML version of the documentation, and folder `test` files implementing unit tests for functions and objects that do not require special hardware, i.e., spectrometers, for execution.
...
Ylianttila, Lasse, Visuri, Reijo, Huurto, Laura, Jokela, Kari (2005) Evaluation of a single-monochromator diode array spectroradiometer for sunbed UV-radiation measurements. 
_Photochemistry and Photobiology_ , 81(2), 333-341.
